...
data &tempwork..analytical;
	format year best4.;
	merge demdx.dementia_dt_&mindatayear._&maxyear. (in=a rename=demdx_dt=date keep=dxtypes bene_id year demdx_dt demdx1-demdx26) demdx.ADdrugs_dts_0619 (in=b rename=srvc_dt=date);
...
data &tempwork..analytical1;
	merge &tempwork..analytical (in=a) &datalib..bene_demog&maxyear. (in=b keep=bene_id death_date);
	by bene_id;
	if a and b;
run;
...
%macro inc_1yrv;
data &outlib..adrdincv1yrv_scen_first;
...
data &outlib..adrdincv1yrv_scendx_long;
	set &tempwork..analytical1;
	by bene_id year date;
...
data &outlib..adrdincv1yrv_scendxrx_long;
	set &tempwork..analytical1;
	by bene_id year date;
...
data &outlib..adrdincv1yrv_scendxsymp_long;
	set &tempwork..analytical1;
	by bene_id year date;
...
/* Split out each into year and keep last.bene_id */

data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scenfirst_&yr. (keep=bene_id first_dx&yr. first_adrx&yr. first_symptoms&yr. first_symptoms_desc&yr. first_adrd_type&yr.)
	 %end;;
	set &outlib..adrdincv1yrlb_scen_first;
	by bene_id;
run;

data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendx_&yr. (keep=bene_id scen_dx_inc&yr. scen_dx_vdt&yr. scen_dx_vtime&yr. scen_dx_inctype&yr. scen_dx_vtype&yr. dropdx&yr.)
	 %end;;
	set &outlib..adrdincv1yrv_scendx_long;
	by bene_id;
	if last.bene_id;
run;

data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendxrx_&yr. (keep=bene_id scen_dxrx_inc&yr. scen_dxrx_vdt&yr. scen_dxrx_vtime&yr. scen_dxrx_inctype&yr. scen_dxrx_vtype&yr. dropdxrx&yr.)
	 %end;;
	set &outlib..adrdincv1yrv_scendxrx_long;
	by bene_id;
	if last.bene_id;
run;

data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendxsymp_&yr. (keep=bene_id scen_dxsymp_inc&yr. scen_dxsymp_vdt&yr. scen_dxsymp_vtime&yr. scen_dxsymp_inctype&yr. scen_dxsymp_vtype&yr. dropdxsymp&yr.)
	 %end;;
	set &outlib..adrdincv1yrv_scendxsymp_long;
	by bene_id;
	if last.bene_id;
run;

data %do yr=&minyear. %to &maxyear.;
	 &outlib..adrdinc_dxrxsymp_yrly_1yrv&yr. (keep=bene_id scen_dxrxsymp_inc&yr. scen_dxrxsymp_vdt&yr. scen_dxrxsymp_vtime&yr. scen_dxrxsymp_inctype&yr. scen_dxrxsymp_vtype&yr. dropdxrxsymp&yr.)
	 %end;;
...
proc freq data=&outlib..adrdinc_dxrxsymp_yrly_1yrv&yr. noprint;
	table scen_dxrxsymp_inctype&yr.*scen_dxrxsymp_vtype&yr. / out=&tempwork..check_type&yr.;
run;

proc univariate data=&outlib..adrdinc_dxrxsymp_yrly_1yrv&yr. noprint outtable=&tempwork..check_vtime&yr.;
...
/*
%do yr=&minyear. %to &maxyear.;
data &outlib..adrdinc_verified_1yrv_&yr.;
	merge &tempwork..scenfirst_&yr. &tempwork..scendx_&yr. &tempwork..scendxrx_&yr. &tempwork..scendxsymp_&yr.  &tempwork..scendxrxsymp_&yr.;
	by bene_id;
run;
