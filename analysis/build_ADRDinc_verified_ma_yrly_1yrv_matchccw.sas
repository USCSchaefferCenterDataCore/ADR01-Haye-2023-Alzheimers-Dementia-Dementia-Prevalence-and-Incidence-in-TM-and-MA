...
data &tempwork..analyticalma;
	format year best4.;
	merge demdx.dementia_dt_ma15_18 (in=a rename=clm_thru_dt=date keep=dxtypes bene_id clm_thru_dt demdx1-demdx26) demdx.ADdrugs_dts_0619 (in=b rename=srvc_dt=date where=(&minyear.<=year(date)<=&maxyear.));
	by bene_id date;
...
proc sort data=&tempwork..analyticalma; by bene_id year date; run;

data &tempwork..analyticalma1;
	merge &tempwork..analyticalma (in=a) &datalib..bene_demog&demogyear. (in=b keep=bene_id death_date);
	by bene_id;
	if a and b;
run;
...
data &outlib..adrdincv1yrv_scendxrxsymp_longma;
	set &tempwork..analyticalma1;
	by bene_id year date;
...
data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scenfirst_&yr.ma (keep=bene_id first_dx&yr. first_adrx&yr. first_symptoms&yr. first_symptoms_desc&yr. first_adrd_type&yr.)
	 %end;;
...
data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendx_&yr.ma (keep=bene_id scen_dx_inc&yr. scen_dx_vdt&yr. scen_dx_vtime&yr. scen_dx_inctype&yr. scen_dx_vtype&yr. dropdx&yr.)
	 %end;;
...
data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendxrx_&yr.ma (keep=bene_id scen_dxrx_inc&yr. scen_dxrx_vdt&yr. scen_dxrx_vtime&yr. scen_dxrx_inctype&yr. scen_dxrx_vtype&yr. dropdxrx&yr.)
	 %end;;
...
data %do yr=&minyear. %to &maxyear.;
	 &tempwork..scendxsymp_&yr.ma (keep=bene_id scen_dxsymp_inc&yr. scen_dxsymp_vdt&yr. scen_dxsymp_vtime&yr. scen_dxsymp_inctype&yr. scen_dxsymp_vtype&yr. dropdxsymp&yr.)
	 %end;;
...
data %do yr=&minyear. %to &maxyear.;
	 &outlib..adrdinc_dxrxsymp_yrly_1yrv&yr.ma (keep=bene_id scen_dxrxsymp_inc&yr. scen_dxrxsymp_vdt&yr. scen_dxrxsymp_vtime&yr. scen_dxrxsymp_inctype&yr. scen_dxrxsymp_vtype&yr. dropdxrxsymp&yr.)
	 %end;;
	set &outlib..adrdincv1yrv_scendxrxsymp_longma;
	by bene_id; 
	if last.bene_id;
run;
...
/* Checks */
%do yr=&minyear. %to &maxyear.;
proc freq data=&outlib..adrdinc_dxrxsymp_yrly_1yrv&yr.ma noprint;
	table scen_dxrxsymp_inctype&yr.*scen_dxrxsymp_vtype&yr. / out=&tempwork..check_type&yr.ma;
run;

proc univariate data=&outlib..adrdinc_dxrxsymp_yrly_1yrv&yr.ma noprint outtable=&tempwork..check_vtime&yr.ma;
