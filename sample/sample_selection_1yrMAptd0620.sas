...
%do year=&mindatayear %to &maxdatayear;
	...
	data &tempwork..bene&year;
		set &datalib..bene_status_year&year (keep=bene_id age_beg enrHMO_allyr enrHMO_mo_yr enrHMO_4_mo_yr ptD_allyr);
		rename age_beg=age_beg&year. enrHMO_allyr=enrHMO_allyr&year. enrHMO_mo_yr=enrHMO_mo_yr&year. enrHMO_4_mo_yr=enrHMO_4_mo_yr&year. ptd_allyr=ptD_allyr&year.;
	run;
%end;
...
%macro sample;
data &outlib..samp_1yrmaptd_0620;
	...
	* race_drop=(race_bg in("","0","3"));    
	...
	%do year=&minsampyear %to &maxsampyear;
		...
		* limiting to age 67 and in ma and Part D in 2 previous years;
		if age_beg&year>=67 
		and dropflag="N"
...
* By year;
%do year=&minsampyear %to &maxsampyear;
proc freq data=&outlib..samp_1yrmaptd_0620 noprint;
	where insamp&year=1;
...
proc means data=&outlib..samp_1yrmaptd_0620 noprint;
	where insamp&year=1;
...
data &tempwork..allages;
	set
	%do year=&minsampyear %to &maxsampyear;
		&outlib..samp_1yrmaptd_0620 (where=(insamp&year=1) keep=insamp&year bene_id age_beg&year rename=(age_beg&year=age_beg))
	%end;;
run;
...
proc export data=samp_stats_maptd
	outfile="&rootpath./Projects/Programs/base/exports/samp_stats_1yrmaptd_0620.xlsx"
	dbms=xlsx
	replace;
	sheet="stats";
run;

proc contents data=&outlib..samp_1yrmaptd_0620; run;

%do year=&minsampyear. %to &maxsampyear.;
proc freq data=&outlib..samp_1yrmaptd_0620;
	where insamp&year.=1;
