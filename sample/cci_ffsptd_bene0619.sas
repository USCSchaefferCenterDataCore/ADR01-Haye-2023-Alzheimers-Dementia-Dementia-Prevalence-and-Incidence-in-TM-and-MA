...
* PURPOSE: Running on CCI on FFS claims;
...
%do year=&minyear. %to &maxyear.;
	...
	data &tempwork..&ctyp._&year._mo&mo.;
		...
		%end;
		base.samp_1yrffsptd_0619 (in=b keep=bene_id anysamp (where=(anysamp=1));*merge;
		by bene_id;
		if a and b;
	run;
	%end;
	...
	data &tempwork..&ctyp._cci_ffsptd&year.;
		set &tempwork..&ctyp._&year._mo1-&tempwork..&ctyp._&year._mo12;
		by bene_id;
	run;
%end; 
...
data base.cci_ffsptd_bene&year.;
...
data base.cci_ffsptd_bene0619;
	merge %do year=&minyear %to &maxyear;
	base.cci_ffsptd_bene&year. (keep=bene_id totalcc wgtcc rename=(totalcc=totalcc&year. wgtcc=wgtcc&year.))
	%end;;
	by bene_id;
run;
