...
proc means data=optum_inc noprint;
	where inc2016 ne .;
	var female age_d2016: race_d: cci2016 inc2016 duallis2016;
	output out=optum_samp2016 sum()= mean()= std(cci2016)= lclm(inc2016)= uclm(inc2016)= / autoname;
run;

proc means data=optum_inc noprint;
	where inc2017 ne .;
	var female age_d2017: race_d: cci2017 inc2017 duallis2017;
...
data incpredict16ses;
	set incpredict16 (in=dl0)
			incpredict16 (in=dl1);
	if dl0 then duallis2016=0;
	if dl1 then duallis2016=1;
run;

data incpredict17ses;
	set incpredict17 (in=dl0)
			incpredict17 (in=dl1);
...
data optum_inc_pred16;
	set optum_inc (rename=(age_d2016_7074=age7074 age_d2016_7579=age7579 age_d2016_ge80=agege80 cci2016=wgtcc)) incpredict16ses (in=b);
	predict=b;
	if b then insamp2016=1;
	rename duallis2016=duallis;
run;

proc reg data=optum_inc_pred16 outest=inc16_optum_linregses;
	where inc2016 ne . or predict;
	model inc2016=female age7074 age7579 agege80 wgtcc race_db race_dh race_da race_do duallis;
	output out=inc16_optum_linpredictses (where=(predict=1) keep=female age7074 age7579 agege80 wgtcc race_d: duallis p lclm uclm predict) predicted=p lclm=lclm uclm=uclm lcl=lcl ucl=ucl;
run;
%export(inc16_optum_linregses,inc16_optum_linregses);
%export(inc16_optum_linpredictses,seinc16_optum_linpredictses);

data optum_inc_pred17;
	set optum_inc (rename=(age_d2017_7074=age7074 age_d2017_7579=age7579 age_d2017_ge80=agege80 cci2017=wgtcc)) incpredict17ses (in=b);
	predict=b;
	if b then insamp2017=1;
	rename duallis2017=duallis;
run;

proc reg data=optum_inc_pred17 outest=inc17_optum_linregses;
	where inc2017 ne . or predict;
	model inc2017=female age7074 age7579 agege80 wgtcc race_db race_dh race_da race_do duallis;
	output out=inc17_optum_linpredictses (where=(predict=1) keep=female age7074 age7579 agege80 wgtcc race_d: duallis p lclm uclm predict) predicted=p lclm=lclm uclm=uclm lcl=lcl ucl=ucl;
run;
%export(inc17_optum_linregses,inc17_optum_linregses);
%export(inc17_optum_linpredictses,inc17_optum_linpredictses);

/**** Weight - use distributions from the FFS Part D ****/
proc sort data=inc16_optum_linpredictses;
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
run;

proc sort data=inc17_optum_linpredictses;
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
run;

proc sort data=ffs.ffsptd_incweights16ses;
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
run;

proc sort data=ffs.ffsptd_incweights17ses;
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
run;

data inc16_weightedses;
	merge inc16_optum_linpredictses (in=a) ffs.ffsptd_incweights16ses (in=b);
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
...
data inc17_weightedses;
	merge inc17_optum_linpredictses (in=a) ffs.ffsptd_incweights17ses (in=b);
	by female agege80 age7579 age7074 race_do race_da race_dh race_db duallis;
	if age7074 then age_group='2. 70-74';
	else if age7579 then age_group='3. 75-79';
	else if agege80 then age_group='4. 80+';
...
run;
%export(inc16_weightedallses,inc16_weightedallses);
%export(inc17_weightedallses,inc17_weightedallses);
...
/*
%incageadj(inc16_weightedses,0,female,female16);
%incageadj(inc16_weightedses,'W',race,race16);
%incageadj(inc17_weightedses,0,female,female17);
%incageadj(inc17_weightedses,'W',race,race17);
